...
class ConvAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(128, 64, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 64, 14]
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.ConvTranspose1d(64, 32, kernel_size=4, stride=2, padding=1, output_padding=1),  # Output: [batch_size, 32, 28]
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.ConvTranspose1d(32, 16, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 16, 56]
            nn.ReLU(),
            nn.BatchNorm1d(16),
...
# Define the custom mean quartic error loss function
def mean_quartic_error(recon, target):
    return torch.mean((recon - target) ** 3)
...
model.load_state_dict(torch.load('best_autoencoder_CNN.pth'))
model.eval()
...
for (device_id, CV) in List_of_CVs:
    ...
    if time_diff:
        ...
        with torch.no_grad():
            recon = model(tensor)
            loss = mean_quartic_error(recon, tensor)
...
# Count the number of blue and red devices with loss over e^(-6)
blue_count = 0
red_count = 0
threshold = np.exp(-9.27)

for device_id, loss in device_losses.items():
    cv = next(cv for (id, cv) in List_of_CVs if id == device_id)
    if loss > threshold:
        if cv > 0.23:
            red_count += 1
        else:
...
print(f'Number of blue devices with loss over e^{np.log(threshold)}: {blue_count}, false positives = {int(1000*(blue_count / 401))/10}%')
print(f'Number of red devices with e^{np.log(threshold)}: {red_count}', f'true positive = {int(100*(red_count / 24))}%')

# Plot the sorted losses with colors based on CV
plt.figure(figsize=(12, 6))
colors = ['red' if cv > 0.28 else 'blue' for device_id, cv in List_of_CVs]
plt.bar(device_losses.keys(), np.log(np.abs(np.array(list(device_losses.values())))), color=colors)
