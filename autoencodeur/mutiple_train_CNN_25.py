...
def pad_or_truncate(seq, target_length=48):
...
for device in List_of_devices:
    ...
    if time_diff:
        ...
        time_diff_padded = pad_or_truncate(time_diff, 48)
...
class ConvAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 16, 25]
            nn.ReLU(),
            nn.BatchNorm1d(16),
            nn.Conv1d(16, 32, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 32, 13]
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.Conv1d(32, 64, kernel_size=3, stride=1, padding=1),  # Output: [batch_size, 64, 13]
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.Conv1d(64, 128, kernel_size=3, stride=1, padding=1),  # Output: [batch_size, 128, 13]
            nn.ReLU(),
            nn.BatchNorm1d(128)
        )
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(128, 64, kernel_size=3, stride=1, padding=1),  # Output: [batch_size, 64, 13]
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.ConvTranspose1d(64, 32, kernel_size=3, stride=1, padding=1),  # Output: [batch_size, 32, 13]
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.ConvTranspose1d(32, 16, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 16, 25]
            nn.ReLU(),
            nn.BatchNorm1d(16),
            nn.ConvTranspose1d(16, 1, kernel_size=4, stride=2, padding=1),  # Output: [batch_size, 1, 50]
            nn.Sigmoid()
        )
...
for epoch in range(num_epochs):
    ...
    if average_loss_on_devices < best_val_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), 'best_autoencoder_CNN_50.pth')
    else:
        patience_counter += 1
