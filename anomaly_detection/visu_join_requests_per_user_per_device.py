...
number_of_devices_per_user = {'Alpha': 50, 'Beta': 76, 'Delta': 51, 'Epsilon': 14, 
                              'Eta': 10, 'Evil': 5, 'Gamma': 132, 'Iota': 14, 'Kappa': 14, 
                              'Lambda': 11, 'Mu': 9, 'Nu': 15, 'Theta': 8, 'Zeta': 18 }
...
# Calculate join requests per device
...
for i, date in enumerate(dates):
    if date in count_per_hour_date_user.index:
        ...
        axes[i].set_title(f'Join requests per device on {date}')
        axes[i].set_xlabel('Hour of the Day')
        axes[i].set_ylabel('Join Requests per Device')
